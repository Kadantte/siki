...
func setupTestDirs(t *testing.T) func() {
	...
	origDigestConfigDir := digestConfigDir
	...
	return func() {
		...
		digestConfigDir = origDigestConfigDir
	}
}
...
// ============================================================================
// Docker Run Model Routing Tests
// ============================================================================

func TestDockerRunModelRouting(t *testing.T) {
	tests := []struct {
		msg  string
		want string
	}{
		{"https://huggingface.co/Lightricks/LTX-2.3 これ使って猫の動画生成して", "docker_run_model"},
		{"https://github.com/D4Vinci/Scrapling 動かしてみて", "docker_run_model"},
		{"huggingface.co/some/model を試してみたい", "docker_run_model"},
		{"https://huggingface.co/meta-llama/Llama-2-7b で生成して", "docker_run_model"},
		{"https://example.com/page を見せて", "web_fetch"},                         // Not HF/GitHub
		{"https://huggingface.co/some/model", "web_fetch"},                         // No action word → web_fetch
		{"猫の画像を生成して", "generate_image"},                                          // No URL
		{"ニュースを教えて", "web_search"},                                                // No URL
		{"twitterでAIについて検索して", "twitter_search"},                                    // Twitter keyword
		{"ツイッターでLLMの最新情報を調べて", "twitter_search"},                                    // ツイッター keyword
		{"最近のツイートでGPT-5について何か言ってる？", "twitter_search"},                              // ツイート keyword
		{"x.comで話題になってることは？", "twitter_search"},                                      // x.com keyword
		{"自分のタイムラインを取得してまとめてほしい", "twitter_timeline"},                                // Timeline keyword
		{"タイムラインの最新情報を教えて", "twitter_timeline"},                                      // タイムライン keyword
		{"Twitterのフィードを見せて", "twitter_timeline"},                                     // フィード keyword
	}

	for _, tt := range tests {
		t.Run(tt.msg, func(t *testing.T) {
			got := detectToolFromKeywords(tt.msg)
			if got != tt.want {
				t.Errorf("detectToolFromKeywords(%q) = %q, want %q", tt.msg, got, tt.want)
			}
		})
	}
}

func TestDockerRunModelURLExtraction(t *testing.T) {
	tests := []struct {
		msg     string
		wantURL string
	}{
		{
			"https://huggingface.co/Lightricks/LTX-2.3 これ使って猫の動画生成して",
			"https://huggingface.co/Lightricks/LTX-2.3",
		},
		{
			"https://github.com/some/repo を動かして",
	...
	for _, tt := range tests {
		t.Run(tt.msg, func(t *testing.T) {
			...
			// Simulate overrideToolArgs logic for docker_run_model
			for _, word := range strings.Fields(tt.msg) {
			...
			if got, ok := args["url"].(string); !ok || got != tt.wantURL {
				t.Errorf("extracted URL = %q, want %q", got, tt.wantURL)
			}
		})
	}
}
...
// ============================================================================
// Twitter Timeline Tests
// ============================================================================

func TestFetchTwitterTimeline(t *testing.T) {
	...
	mockServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if strings.Contains(r.URL.Path, "/users/me") && !strings.Contains(r.URL.Path, "timelines") {
			// /2/users/me
			json.NewEncoder(w).Encode(map[string]interface{}{
		...
		if strings.Contains(r.URL.Path, "timelines/reverse_chronological") {
			// Timeline response
			json.NewEncoder(w).Encode(map[string]interface{}{
				"data": []map[string]string{
					{"id": "t1", "text": "New GPT-5 released today! #AI #LLM", "created_at": "2026-03-17T08:00:00Z", "author_id": "u1"},
					{"id": "t2", "text": "Had a great lunch at the ramen shop", "created_at": "2026-03-17T07:00:00Z", "author_id": "u2"},
					{"id": "t3", "text": "Diffusion models are getting crazy fast https://arxiv.org/xxx", "created_at": "2026-03-17T06:00:00Z", "author_id": "u1"},
				},
				"includes": map[string]interface{}{
					"users": []map[string]string{
						{"id": "u1", "name": "AI Researcher", "username": "airesearcher"},
						{"id": "u2", "name": "Food Lover", "username": "foodlover"},
					},
				},
				"meta": map[string]int{"result_count": 3},
			})
			return
		}
	...
	// We can't easily override the Twitter API URL in fetchTwitterTimeline since it's hardcoded,
	// so we test the parsing logic by simulating the response.
	// Instead, test the components: user map building, tweet parsing

	t.Run("parse_twitter_response", func(t *testing.T) {
		responseJSON := `{
			"data": [
				{"id": "t1", "text": "New GPT-5 released!", "created_at": "2026-03-17T08:00:00Z", "author_id": "u1"},
				{"id": "t2", "text": "Nice weather today", "created_at": "2026-03-17T07:00:00Z", "author_id": "u2"}
			],
			"includes": {
				"users": [
					{"id": "u1", "name": "AI Researcher", "username": "airesearcher"},
		...
		if err := json.Unmarshal([]byte(responseJSON), &apiResp); err != nil {
		...
		if userMap["u1"] != "AI Researcher (@airesearcher)" {
			t.Errorf("unexpected user map: %v", userMap)
		}
		...
		if tweets[0].Author != "AI Researcher (@airesearcher)" {
			t.Errorf("expected expanded author, got %q", tweets[0].Author)
		}
		if tweets[0].Text != "New GPT-5 released!" {
			t.Errorf("unexpected tweet text: %q", tweets[0].Text)
		}
	})
}

func TestFilterTwitterPromptFormat(t *testing.T) {
	// Test that the prompt is correctly built from tweets (without calling LLM)
	tweets := []TwitterTweet{
		{ID: "1", Text: "LLM benchmarks show 50% improvement", CreatedAt: "2026-03-17T08:00:00Z", Author: "AI News (@ainews)"},
		{ID: "2", Text: "Great sushi dinner", CreatedAt: "2026-03-17T07:00:00Z", Author: "Food Guy (@food)"},
		{ID: "3", Text: "New diffusion model paper https://arxiv.org/123", CreatedAt: "2026-03-17T06:00:00Z", Author: "ML Daily (@mldaily)"},
	}

	var sb strings.Builder
	for _, tw := range tweets {
		sb.WriteString(fmt.Sprintf("[%s] %s: %s\n\n", tw.CreatedAt, tw.Author, tw.Text))
	}
	prompt := sb.String()

	if !strings.Contains(prompt, "AI News (@ainews)") {
		t.Error("expected author in prompt")
	}
	if !strings.Contains(prompt, "LLM benchmarks") {
		t.Error("expected tweet text in prompt")
	}
	if !strings.Contains(prompt, "https://arxiv.org/123") {
		t.Error("expected URL preserved in prompt")
	}
	if !strings.Contains(prompt, "Great sushi dinner") {
		t.Error("expected non-AI tweet in raw prompt (filtering is done by LLM)")
	}
}

func TestDigestSettingsTwitter(t *testing.T) {
	cleanup := setupTestDirs(t)
	defer cleanup()

	// Create a test config
	config := defaultConfig()
	ws := &WebServer{config: config}

	// POST Twitter settings
	body := `{"twitter_enabled":true,"twitter_bearer_token":"AAAA_test_token_1234","digest_enabled":true,"email_to":"test@test.com","smtp_host":"smtp.test.com","smtp_port":587}`
	req := httptest.NewRequest("POST", "/api/digest/settings", strings.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	ws.handleDigestSettings(w, req)
	...
	if !ws.config.TwitterEnabled {
		t.Error("TwitterEnabled should be true")
	}
	if ws.config.TwitterBearerToken != "AAAA_test_token_1234" {
		t.Errorf("unexpected token: %q", ws.config.TwitterBearerToken)
	}

	// GET should return masked token
	req2 := httptest.NewRequest("GET", "/api/digest/settings", nil)
	w2 := httptest.NewRecorder()
	ws.handleDigestSettings(w2, req2)

	var resp map[string]interface{}
	json.Unmarshal(w2.Body.Bytes(), &resp)

	if resp["twitter_enabled"] != true {
		t.Error("expected twitter_enabled=true in GET response")
	}
	maskedToken, _ := resp["twitter_bearer_token"].(string)
	if maskedToken != "AAAA...1234" {
		t.Errorf("expected masked token 'AAAA...1234', got %q", maskedToken)
	}

	// POST with masked token should NOT overwrite the real token
	body2 := `{"twitter_enabled":true,"twitter_bearer_token":"AAAA...1234","digest_enabled":true,"email_to":"test@test.com"}`
	req3 := httptest.NewRequest("POST", "/api/digest/settings", strings.NewReader(body2))
	req3.Header.Set("Content-Type", "application/json")
	w3 := httptest.NewRecorder()
	ws.handleDigestSettings(w3, req3)

	if ws.config.TwitterBearerToken != "AAAA_test_token_1234" {
		t.Errorf("masked token POST should not overwrite real token, got %q", ws.config.TwitterBearerToken)
	}
}
